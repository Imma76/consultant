...
class FirebaseMessageService{
  static Future sendMessage({required String message, required String user, required String receiver, required String receiverNo})async{
    await FirebaseFirestore.instance.collection('users').doc(userController.consultant!.phoneNumber).collection('Dr${userController.consultant!.firstName} messaged').add({'message':message, 'user':user, 'receiver':receiver});
  ...
  static  addConversationMessages(var chatRoomId, var message)async{
    await FirebaseFirestore.instance.collection('chatroom').doc(chatRoomId).collection('chats').add({'message':message,
      'sendBy':'${userController.consultant!.firstName}','time':DateTime.now().millisecondsSinceEpoch}).catchError((error)=>print(error)) ;
  }
